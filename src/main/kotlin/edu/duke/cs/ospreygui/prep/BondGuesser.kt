...
fun Molecule.guessBonds(
	tolerance: Double = 0.45,
	minDist: Double = 0.4,
	searchDist: Double = 5.0,
	elementProps: EnumMap<Element,ElementProps> = defaultElementProps()
): List<Bond> {

	val minDistSq = minDist.square()
	val searchDistSq = searchDist.square()

	fun Atom.toPoint() = doubleArrayOf(pos.x, pos.y, pos.z)

	// build a KD tree of the atoms so we can do nearest-neighbor queries efficiently
	val tree = KdTree<Atom>(3).apply {
		for (atom in atoms) {
			addPoint(atom.toPoint(), atom)
		}
	}
	...
	val guessedBonds = ArrayList<Bond>()

	val distFunc = SquareEuclideanDistanceFunction()
	for (atom1 in atoms) {
		val props1 = atom1.props()

		// find the closest neighbors, up to the number allowed by the max valence
		val nearbyAtoms = tree.getNearestNeighborIterator(
			atom1.toPoint(),
			Int.MAX_VALUE,
			distFunc
		)
		while (bonds.bondedAtoms(atom1).size < props1.maxValence) {
			...
			val atom2 = nearbyAtoms.next()
			val props2 = atom2.props()
			val distSq = atom1.pos.distanceSquared(atom2.pos)
			...
			val covDistSq = (props1.bondRadius + props2.bondRadius + tolerance).square()
			if (distSq > covDistSq) {
				continue
			}

			// atom is in range, make the bond
			guessedBonds.add(Bond(atom1, atom2))
		}
	}
